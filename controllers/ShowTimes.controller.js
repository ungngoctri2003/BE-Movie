...
const create = async (req, res) => {
  ...
  try {
    // Thêm điều kiện thời gian tạo lịch chiếu phải lớn hơn hoặc bằng ngày hiện tại
    const currentDate = new Date();
    if (new Date(showDate) < currentDate) {
      return res
        .status(400)
        .send("Thời gian tạo lịch chiếu phải lớn hơn hoặc bằng ngày hiện tại");
    }
...
const showtimesWithGroupCinemas = async (req, res) => {
  ...
  try {
    const listGroupCinema = await sequelize.query(
      `select * from groupcinemas where isActive = true `,
      { type: QueryTypes.SELECT }
    );
    let hack = [];
        ...
        if (idFilm === "") {
          listFilm = await sequelize.query(
            `
                    ...
                    where showtimes.idCinema = ${cinema.id}`,
            { type: QueryTypes.SELECT }
          );
        ...
        for (const film of listFilm) {
          const lstShowDate = await sequelize.query(
            `select showtimes.id,  showDate
                    ...
                    where cinemas.id = ${cinema.id} and films.id = ${film.idFilm} and showtimes.isActive = true
                      and showtimes.showDate >= CURDATE()`, // Add the condition here
            { type: QueryTypes.SELECT }
          );
  ...
  try {
    const lstShowDate = await sequelize.query(
      `
                ...
                inner join films on films.id = showtimes.idFilm
                inner join cinemas on cinemas.id = showtimes.idCinema
                where showtimes.isActive = true and cinemas.id = ${idCinema} and films.id = ${idFilm}`,
      { type: QueryTypes.SELECT }
    );
